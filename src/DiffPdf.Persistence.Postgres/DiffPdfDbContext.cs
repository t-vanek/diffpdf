...
public sealed class DiffPdfDbContext(DbContextOptions<DiffPdfDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<TriggerEntity>(e =>
        {
            ...
            e.HasIndex(x => x.InstanceId).IsUnique().HasFilter("is_default AND NOT is_deleted");
        });
        ...
        b.Entity<ScopeConfigurationEntity>(e =>
        {
            ...
            e.Property(x => x.BranchId).HasColumnName("branch_id");
            e.Property(x => x.InstanceId).HasColumnName("instance_id");
            e.Property(x => x.TriggerSource).HasColumnName("trigger_source");
            e.Property(x => x.TriggerConfigJson).HasColumnName("trigger_config_json").HasColumnType("jsonb");
            e.Property(x => x.ComparerSource).HasColumnName("comparer_source");
            e.Property(x => x.ComparisonOptionsJson).HasColumnName("comparison_options_json").HasColumnType("jsonb");
            e.Property(x => x.CreatedAt).HasColumnName("created_at");
            e.Property(x => x.UpdatedAt).HasColumnName("updated_at");
            e.Property(x => x.Version).HasColumnName("version");
